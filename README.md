...
| --------------------------------- | ------------------------------------------------------- | ------------------------------------------------ |
| `app.mihon.hmos`         | HMOS bundle name (reverse-DNS, ends in `.hmos`)         | `com.beemdevelopment.aegis.hmos`                 |
| `Mihon`              | Vendor / publisher name (usually app name)              | `Aegis`                                          |
| `1`        | Integer version code                                    | `1`                                              |
| `1.0.0`        | Semver version string                                   | `1.0.0`                                          |
| `Mihon`           | Display name shown on the launcher                      | `Aegis`                                          |
| `mihon`      | Lowercase, no-space identifier for `oh-package.json5`   | `aegis`                                          |
| `Mihon manga reader for HarmonyOS` | One-line project description                            | `Aegis 2FA authenticator for HarmonyOS`          |
| `Mihon main module`         | Description for `entry/src/main/module.json5` (module)  | `Aegis main module`                              |
| `Mihon main entry ability`        | Description for `entry/src/main/module.json5` (ability) | `Aegis main entry ability`                       |
...
find "${DEST}" -type f \( -name '*.json5' -o -name '*.json' -o -name '*.ts' -o -name '*.ets' -o -name '*.md' \) \
  -exec sed -i \
    -e 's|app.mihon.hmos|com.beemdevelopment.aegis.hmos|g' \
    -e 's|Mihon|Aegis|g' \
    -e 's|1|1|g' \
    -e 's|1.0.0|1.0.0|g' \
    -e 's|Mihon|Aegis|g' \
    -e 's|mihon|aegis|g' \
    -e 's|Mihon manga reader for HarmonyOS|Aegis 2FA authenticator for HarmonyOS|g' \
    -e 's|Mihon main module|Aegis main module|g' \
    -e 's|Mihon main entry ability|Aegis main entry ability|g' \
    {} +
```
...
$map = @{
  'app.mihon.hmos'         = 'com.beemdevelopment.aegis.hmos'
  'Mihon'              = 'Aegis'
  '1'        = '1'
  '1.0.0'        = '1.0.0'
  'Mihon'           = 'Aegis'
  'mihon'      = 'aegis'
  'Mihon manga reader for HarmonyOS' = 'Aegis 2FA authenticator for HarmonyOS'
  'Mihon main module'         = 'Aegis main module'
  'Mihon main entry ability'        = 'Aegis main entry ability'
}
